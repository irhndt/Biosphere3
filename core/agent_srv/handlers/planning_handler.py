...
class PlanningHandler(BaseHandler):
    async def generate_daily_objective(self, state: RunningState):
        await asyncio.gather(
            self.generate_general_daily_objective(state),
            self.generate_trading_objective(state),
        )

        await self.merge_objectives(state)

    async def generate_general_daily_objective(self, state: RunningState):
    ...
    async def generate_trading_objective(self, state: RunningState):
        obj_planner = self.create_planner(
    ...
    async def merge_objectives(self, state: RunningState):
        # merge objectives
        merger = self.create_planner(
            merger_prompt,
            state.get("character_stats", {}).get("model_type"),
            DailyObjective,
            0.7,
        ...
        daily_objectives_list = list(state["decision"]["daily_objective"])
        last_daily_objective = daily_objectives_list[
            -1
